...
#elif defined(NASOQ_EMBEDDED)
   nasoq::embedded::dsyr("L", &dimx, &diag, tmp1, &iun, tmp1_stride, &stride);
#else
...
#elif defined(NASOQ_EMBEDDED)
    nasoq::embedded::dscal(&n_rhs, &tmp, rhs + i * lda, &iun);
#else
...
#elif defined(NASOQ_EMBEDDED)
    nasoq::embedded::dscal(&n_rhs, &one_over_det, rhs + i * lda, &iun);
    nasoq::embedded::dscal(&n_rhs, &one_over_det, rhs + (i + 1) * lda, &iun);
#else
