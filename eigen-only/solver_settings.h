...
/* ---------------------------------------------------------
   SolverSettings class that handles H,q + eq/ineq
   --------------------------------------------------------- */
...
public:
  ...
  // Big “KKT” or “super” matrix for H + eq + ineq
  CSC *A_ord; 
  CSC *AT_ord; 
  LFactor *L;
  double *D;    
  ...
  // For building big matrix
  CSC *KKT=nullptr; // store H + eq + ineq

  SolverSettings(CSC *H_, double *q_,
                 CSC *A_, double *b_,
                 CSC *C_, double *l_, double *u_)
   : H(H_), q(q_), A(A_), b(b_), C(C_), l(l_), u(u_),
     A_ord(nullptr), AT_ord(nullptr),
     L(new LFactor), D(nullptr),
     blockSet(nullptr), supNo(1),
     psi(nullptr), x(nullptr), n(0),
     solver_mode(0)
  {
    default_setting();
    if(A && A->nrow>0) solver_mode=1; 
    if(C && C->nrow>0) solver_mode=1; 
    n= H->ncol;
  ...
  ~SolverSettings(){
    ...
    if(x)        delete[] x;
    if(KKT){
      delete[]KKT->p; delete[]KKT->i; delete[]KKT->x; delete KKT;
    }
  }
  ...
  // Build big “KKT” = [H; A; C]
  // Then store in KKT->nrow, KKT->ncol, etc.
  // This merges Hessian (n x n), eq A (me x n),
  // ineq C (mi x n). Then forms dimension = n + me + mi.
  int build_super_matrix(){
    if(!H) return 0;
    int status=0;
    int me= (A? A->nrow : 0);
    int mi= (C? C->nrow : 0);
    int bigN= H->ncol + me + mi; 
    KKT= new CSC;
    KKT->nrow= bigN;
    KKT->ncol= bigN;
    // Count nnz
    // For each col j in [0..n-1], we add nnz from H->p[j+1]-H->p[j]
    // plus from A->p[j+1]-A->p[j], plus from C->p[j+1]-C->p[j].
    // Then for the extra me + mi cols, each is 1 diagonal entry
    // (like a small identity to keep them non-empty).
    int *tmpP= new int[bigN+1]();
    // 1) Accumulate in tmpP[0..n]
    for(int j=0;j<(int)H->ncol;j++){
      int nnzH= H->p[j+1]-H->p[j];
      tmpP[j+1]= tmpP[j]+ nnzH; 
      if(A && me>0){
        tmpP[j+1]+= (A->p[j+1]-A->p[j]);
      }
      if(C && mi>0){
        tmpP[j+1]+= (C->p[j+1]-C->p[j]);
      }
    }
    // 2) For the last me+mi columns, each has +1 diagonal
    for(int j= (int)H->ncol+1; j<= (int)(H->ncol + me + mi); j++){
      tmpP[j]= tmpP[j-1]+1;
    }
    int totNnz= tmpP[bigN];
    KKT->nzmax= totNnz;
    KKT->p= tmpP;
    KKT->i= new int[totNnz];
    KKT->x= new double[totNnz]();
    KKT->stype=-1; // unsymmetric
    KKT->xtype= CHOLMOD_REAL;
    KKT->packed=1; 
    KKT->sorted=1;
    ...
    for(int j=0; j<(int)H->ncol; j++){
      int baseP= KKT->p[j];
      // Copy Hessian column
      int nh= H->p[j+1] - H->p[j];
      for(int kk=0; kk< nh; kk++){
        int row= H->i[ H->p[j]+kk ];
        double val= H->x[ H->p[j]+kk ];
        KKT->i[ baseP ]= row; 
        KKT->x[ baseP ]= val;
        baseP++;
      }
      // Copy eq A if present
      if(A && me>0){
        int na= A->p[j+1]- A->p[j];
        for(int kk=0; kk< na; kk++){
          int row= A->i[ A->p[j]+kk ];
          // shift row by n
          row += H->ncol; // eq block
          double val= A->x[ A->p[j]+kk ];
          KKT->i[ baseP ]= row;
          KKT->x[ baseP ]= val;
          baseP++;
        }
      }
      // Copy ineq C if present
      if(C && mi>0){
        int nc= C->p[j+1] - C->p[j];
        for(int kk=0; kk< nc; kk++){
          int row= C->i[ C->p[j]+kk ];
          row += (H->ncol + me); 
          double val= C->x[ C->p[j]+kk ];
          KKT->i[ baseP ]= row;
          KKT->x[ baseP ]= val;
          baseP++;
        }
      ...
      // done
      // next col = tmpP[j+1]
      assert(baseP == (int)tmpP[j+1]);
    }
    // Diag for the me+mi columns
    int startCol= (int)H->ncol;
    for(int j= startCol; j< bigN; j++){
      int idx= KKT->p[j];
      // we put a small diagonal like 1e-6 or so if needed
      // or 0 if we want to let factor handle
      KKT->i[idx]= j;
      KKT->x[idx]= 1e-12; // small
    }
    return 1;
  }
  ...
  int symbolic_analysis(){
    ...
    if(solver_mode==0){
      ...
      blockSet= new int[2]; blockSet[0]=0; blockSet[1]= A_ord->ncol;
      x= new double[A_ord->ncol];
      for(int i=0;i<(int)A_ord->ncol;i++) x[i]=0.0;
      return 1;
    } else {
      // build super
      build_super_matrix(); // store in KKT
      // then do same allocation
      A_ord= new CSC;
      A_ord->nrow= KKT->nrow;
      A_ord->ncol= KKT->ncol;
      A_ord->nzmax= KKT->nzmax;
      A_ord->p= new int[A_ord->ncol+1];
      A_ord->i= new int[A_ord->nzmax];
      A_ord->x= new double[A_ord->nzmax];
  ...
  double* solve_only(){
    ...
    int nvar= A_ord->ncol;
    // TODO: check this do a forward->diag->back using D
    // and the strictly lower part in L->x
    // assume q is the RHS
    ...
    for(int col=0; col<nvar; col++){
      double val= x[col];
      int st= (int)L->i_ptr[col];
      int ed= (int)L->i_ptr[col+1];
